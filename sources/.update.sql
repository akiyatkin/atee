...
CREATE TABLE IF NOT EXISTS `sources_props` (
  ...
  `known` enum('system','more','column','secondary') NOT NULL DEFAULT 'more' COMMENT 'system показывается в админке shop, чтобы настроить группы, но не показывается нигде в интерфейсе даже в json',
