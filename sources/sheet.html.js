...
export const ROOT = (data, env, sheet = data.sheet, source = data.source) => err(data, env, ['TABLE']) || `
	...
	<div style="margin-bottom:1em; display: grid; grid-template-columns: 1fr; gap: 1em">
		<div>
			${field.area({
				name: 'comment', 
...
const showMultiSpan = (data, env, sheet, source, text, text_index, col_index, multi_index) => `
	<button style="${text ? '' : 'display:block; width:100%'}" class="value transparent ${data.winners[text_index][col_index][multi_index] ? '' : 'mute'}">${text || '&nbsp;'}</button>
`.trim()
