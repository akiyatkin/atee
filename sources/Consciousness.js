...
Consciousness.recalcEntitiesPropId = async (db) => { //Определить entity_id, prop_id, key_index
	...
	//source.entity_id дефолтная сущность, ключ дефолтной сущности
	//source.prop_id 
...
Consciousness.recalcEntitiesPropIdBySource = async (db, source) => { //Определить entity_id, prop_id, key_index
...
Consciousness.recalcRowsKeyIdBySource = async (db, source) => {
	await db.exec(`
		UPDATE sources_rows ro
		SET ro.key_id = null, ro.repeat_index = null
...
Consciousness.recalcMultiBySource = async (db, {source_id}) => { //prop_id может не быть, тогда multi считаем false
	const list = await db.all(`
		SELECT 
			co.sheet_index, 
			co.col_index, 
...
Consciousness.recalcTexts_byPropId = async (db, prop_id) => {
	await db.exec(`
...
Consciousness.recalcTextsBySource = async (db, {source_id}) => {
	await db.exec(`
		UPDATE sources_cells ce, sources_cols co
			LEFT JOIN sources_props pr on (pr.prop_id = co.prop_id)
		SET 
...
Consciousness.recalcRepresentCell = async (db) => {
	await db.exec(`
		...
		WHERE ce.source_id = so.source_id 
	`)
...
Consciousness.recalcRepresentRowSummary = async (db) => {
	// await db.exec(`
	// 	UPDATE sources_cells
...
Consciousness.recalcRepresentRowKey = async (db) => {
	await db.exec(`
		UPDATE sources_rows ro, sources_sources so
...
Consciousness.recalcWinner = async (db) => {
	...
	await db.exec(`
		...
		SELECT 
	 	...
	 	WHERE ce.sheet_index = co.sheet_index and ce.col_index = co.col_index and ce.multi_index = 0
	 		and pr.prop_id = co.prop_id
	...
	await db.exec(`
		INSERT INTO sources_wvalues (
			win_id, value_id, multi_index
		)
		SELECT 
		 	wi.win_id, ce.value_id, ce.multi_index
	 	FROM sources_winners wi, sources_cells ce
	 	WHERE ce.source_id = wi.source_id and ce.sheet_index = wi.sheet_index and ce.row_index = wi.row_index and ce.col_index = wi.col_index
	 		and ce.value_id is not null
	`)
	...
	await db.exec(`
		INSERT INTO sources_wnumbers (
			win_id, number, multi_index
		)
		SELECT 
		 	wi.win_id, ce.number, ce.multi_index
	 	FROM sources_winners wi, sources_cells ce
	 	WHERE ce.source_id = wi.source_id and ce.sheet_index = wi.sheet_index and ce.row_index = wi.row_index and ce.col_index = wi.col_index
	...
	await db.exec(`
		INSERT INTO sources_wdates (
			win_id, date, multi_index
		)
		SELECT 
		 	wi.win_id, ce.date, ce.multi_index
	 	FROM sources_winners wi, sources_cells ce
	 	WHERE ce.source_id = wi.source_id and ce.sheet_index = wi.sheet_index and ce.row_index = wi.row_index and ce.col_index = wi.col_index
	...
	await db.exec(`
		INSERT INTO sources_wtexts (
			win_id, text, multi_index
		)
		SELECT 
		 	wi.win_id, ce.text, ce.multi_index
