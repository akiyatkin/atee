...
export const ROOT = (data, env) => err(data, env) || `
	...
	<div style="display: grid; gap: 1em; margin: 2em 0">
		...
		<div>
			...
			<p>
				<b>Статистика пула</b><br>
				${Object.entries(data.pool).map(([title, value]) => title + ': ' + value + '<br>').join('')}
			</p>
		...
		<div>
			...
			<p>
				...
				${field.prompt({
					type:'area',
					descr: '<mark>Внимание</mark>: перед импортом сделайте резервный экспорт текущих данных! Ранее внесённые данные нужно очистить. Новые данные не добавятся, а заменят старые данные.',
