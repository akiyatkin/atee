	...
	<script type="module" async>
		...
		for (let btn of cls('load')) {
			btn.addEventListener('click',async () => {
				...
				btn.disabled = true
				const ans = await fetch('/-showcase/'+btn.dataset.action+'?name=' + btn.name).then(res => res.json()).catch(e => false)
...
`
const pricerow = ({options, row, file, name, size, mtime, ready}) => `
	<tr>
		<td>
			<b>${name}</b><br>
		...
		<td style="padding-left:5px">
			...
			<div style="display: flex; flex-wrap: wrap; gap:5px">
				<button data-action="set-prices-clear" name="${name}" ${row?.loaded ? '':'disabled'} class="clear">Очистить</button>
				<button data-action="set-prices-load" name="${name}" class="load ${ready?'ready':''}">Внести</button>
			</div>
		</td>
	</tr>
`
