			...
			(btn => {
				btn.addEventListener('click', async () => {
					...
					const popup = await Dialog.open({
						...
						sub:"ROOT",
						json:"/-shop/cart/get-modification?art_nick=${pos.art_nick}&brendart_nick=${pos.brendart_nick}"
					})
					const form = popup.getElementsByTagName('form')[0]
					form.addEventListener('submit', async e => {
						e.preventDefault()
						const senditmsg = await import('/-dialog/senditmsg.js').then(r => r.default)
						const ans = await senditmsg(btn, '/-shop/cart/set-modification?art_nick=${pos.art_nick}&brendart_nick=${pos.brendart_nick}', new FormData(form))
...
tpl.showImage = (data, env, pos, item) => `
	<img alt="${pos.brendart_nick}" width="80" src="${~item.images[0].indexOf(':') ? item.images[0] : '/' + item.images[0]}">
`
// tpl.showImage = (data, env, pos, item) => `
