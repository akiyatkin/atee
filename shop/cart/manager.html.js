		...
		(div => {
			for (const button of div.getElementsByClassName('status')) {
				...
				button.addEventListener('click', async () => {
					...
					const ans = await fetch('/-shop/cart/set-status?' + new URLSearchParams({ status, order_id }).toString()).then(r => r.json()).catch(e => {
						console.log(e)
			...
			for (const button of div.getElementsByClassName('delete')) {
				button.addEventListener('click', async () => {
					...
					const ans = await fetch('/-shop/cart/set-delete?' + new URLSearchParams({ order_id }).toString()).then(r => r.json()).catch(e => {
						console.log(e)
						return {msg:'Ошибка на сервере'}
					})
					const Client = await window.getClient()
...
tpl.buttons = (data, env) => ' ' || `
	<p align="right">
		...
		<script>
			(button => {
				button.addEventListener('click', async () => {
					const ans = await fetch('/-shop/cart/set-manager-refresh?partner').then(r => r.json()).catch(e => {
						console.log(e)
						return {msg:'Ошибка на сервере'}
					})
