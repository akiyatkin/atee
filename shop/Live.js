...
export const Live = {
	...
	fetchCreate: ({query, partner}) => {
		const fetch_promise = createPromise('live promise ' + query)
		const controller = new AbortController()
		const signal = controller.signal
		const promise = fetch('/-shop/get-livemodels?partner=' + partner + '&query='+query, {signal}).then(res => res.json()).catch(e => null)
		promise.then((ans) => {
	...
	init: form => {
		...
		form.addEventListener('submit', async e => {
			...
			// }
			// if (m) {
			// 	Client.pushState(need.hash ? '/catalog?m='+m+':query=' + need.query : '/catalog')	
			// } else {
			// 	Client.pushState(need.hash ? '/catalog?m=query=' + need.query : '/catalog')	
			// }
