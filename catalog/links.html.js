...
links.model = (data, env, mod) => `${env.crumb || '/catalog'}/${mod.brand_nick}/${mod.model_nick}${links.setm(data)}`
links.brand = (data, env, mod) => `${env.crumb || '/catalog'}/${mod.brand_nick}${links.setm(data)}`
links.search = (val, env) => `${env.crumb || '/catalog'}/${nicked(val)}`
...
links.val = (data, env, mod, pr, val) => `${env.crumb || '/catalog'}/${links.addm(data)}more.${pr.prop_nick}::.${nicked(val ?? common.prnick(mod, pr))}=1`
