...
Bed.getItemByNick = async (db, base, brand_nick, model_nick, item_num, partner) => {
	...
	return cache.konce('getItemByNick', model_id + ':' + item_num + ':' + partner, async () => {
		...
		const models = await Bed.getModelsByItems(db, base, moditem_ids, partner)
		const item = models[0]
		//unset(item.items)
		return item
	})
}
Bed.getPropById = async (db, prop_id) => {
...
Bed.getEntityByTitle = async (db, entity_title) => {
	...
	const entity = await db.fetch(`
		SELECT 
			...
			so.entity_title, 
			so.entity_plural
		FROM sources_entities so
		WHERE so.entity_nick = :entity_nick
	`, {entity_nick})
	return entity
}
Bed.getPropByTitle = async (db, prop_title) => {
	const prop_nick = nicked(prop_title)
	return Bed.getPropByNick(db, prop_nick)
}
Bed.getPropByNick = async (db, prop_nick) => {
	const conf = await config('bed')
	const entity_id = Bed.getEntityByTitle(conf.position_entity_title)
	const prop = await db.fetch(` //column known не хватает
		SELECT 
			pr.prop_nick, 
			pr.prop_title, 
			pr.prop_id, 
...
Bed.mdvalues = async (view, base, md, res = {}) => {
	const mdvalues = {}
	const mdprops = {}
	for (const prop_nick in md.more) {
		...
		if (md.more[prop_nick] == 'empty') {
			...
			for (const value_nick in md.more[prop_nick]) {
				if (prop.type == 'value') {
					mdvalues[value_nick] = await Bed.getValueByNick(db, value_nick)
				} else {
					let unit = ''
...
Bed.getModelsByItems = async (db, base, moditems_ids, partner) => { //[{item_nums after GROUP_CONCAT(distinct i.item_num separator ','), model_id}]
	...
	for (const model of list) {
		for (const item of model.items) {
			for (const prop_title in item) {
				...
				const prop = await Bed.getPropByTitle(db, prop_title)
	...
	for (const model of list) {		
		let { props } = await Bed.getGroupOpt(db, visitor, model.group_id)
		...
		model.card_props = await filter(model.card_props, async (pr) => {
			...
			const ppp = await Bed.getPropByTitle(db, p)
			let ar = []
	...
	const cost = await Bed.getPropByTitle(db, 'Цена')
	const oldcost = await Bed.getPropByTitle(db, 'Старая цена')
	for (const model of list) {
		await Bed.prepareCost(base, model, partner)
		await Bed.prepareCostMinMax(base, model)
...
Bed.prepareCost = async (base, model, partner) => {
	partner = partner || {}
	/*
...
Bed.prepareCostMinMax = async (base, model) => {
...
Bed.getGroupOpt = async (db, visitor, group_id) => {
	const options = await Showcase.getOptions(visitor)
	const group = await Bed.getGroupById(db, visitor, group_id)
	let opt = {}
...
Bed.getGroupById = async (db, visitor, id) => {
	const groups = await Bed.getTree(db, visitor)
	return groups[id]
}

Bed.getTree = async (db, visitor) => {
	//const conf = await config('bed')
	const cache = Access.relate(Bed)
...
Bed.getMainGroups = async (view, prop_title = '') => {
	const { db, base, options } = await view.gets(['db', 'options','base'])
	const cache = base.dbcache
...
Bed.getFilterConf = async (view, prop, group_id, md, partner) => {
	const { db, base, options } = await view.gets(['db', 'options','base'])
	const cache = base.dbcache
	const group = await Bed.getGroupById(db, view.visitor, group_id)
	...
	if (selected) {
		...
		for (const value_nick of values_nicks) {
			if (prop.type == 'value') {
				...
				if (filter.values.some(v => v.value_nick == value_nick)) continue
				const brand = await Bed.getBrandByNick(db, value_nick)
	...
	nmd.m = Bed.makemark(nmd).join(':')

	const {from, where} = await Bed.getmdwhere(view, nmd, partner)
	if (prop.type == 'value') {
...
Bed.getValueById = async (db, value_id) => {
	return db.fetch(`
		SELECT value_nick, value_title, value_id 
		FROM sources_values WHERE value_id = :value_id
	`, { value_id })
}
Bed.getValueByNick = async (db, value_nick) => {
	return db.fetch(`
		SELECT value_nick, value_title, value_id 
		FROM sources_values WHERE value_nick = :value_nick
	`, { value_nick })
}
...
Bed.getPathNickByGroupId = async (view, id) => {
	const db = await view.get('db')
	const group = await Bed.getGroupById(db, view.visitor, id)
...
Bed.getBrandByNick = async (db, nick) => {
	const brands = await Bed.getBrands(db)
	return brands[nick]
}
Bed.getBrands = async (db) => {
	const cache = Access.relate(Bed)
...
Bed.getModelByNick = async (view, brand_nick, model_nick, partner = '') => {
	const { base, db } = await view.gets(['base', 'db'])
	const cache = base.vicache
	const brand_id = await base.getBrandIdByNick(brand_nick)
	const model_id = await base.getModelIdByNick(brand_id, model_nick)
...
Bed.getmdwhere = async (view, md, partner = '') => {
	const { base, db } = await view.gets(['base', 'db'])
	const cache = base.vicache

	return cache.konce('getmdwhere', md.m, async () => {
		const groupnicks = await Bed.getGroups(view)
		const brandnicks = await Bed.getBrands(db)
		const where = []
		if (md.group) {
			let group_ids = []
		...
		if (md.more) {
			...
			for (const prop_nick in md.more) {
				...
				if (partner?.cost && prop_nick == 'cena') {
					prop = await Bed.getPropByTitle(db, partner.cost)
				} else {
					prop = await Bed.getPropByNick(db, prop_nick)
				}
...
Bed.getGroupIds = async (view, md, partner) => {
	const { db } = await view.gets(['db'])
	const {where, from} = await Bed.getmdwhere(view, md, partner)
...
Bed.getMainGroup = async (view, md) => {
