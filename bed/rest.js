...
import rest_live from '/-bed/rest.live.js'
...
rest.addResponse('get-model-head', async (view) => {
	...
	if (!model) {
		view.data.brand = await Bed.getBrandByNick(db, brand_nick)
...
rest.addResponse('get-filters', async (view) => {
	...
	view.data.m = md.m
	view.data.md = md
	const group = await Bed.getMainGroup(view, md)
	if (!group) return view.err('Нет данных')
	const res = {}
	const opt = await Bed.getGroupOpt(db, view.visitor, group.group_id)
	const filters = []
	...
	if (md.more) for (const prop_nick in md.more) {
		...
		const filter = await Bed.getFilterConf(view, prop, group.group_id, md, partner)
		if (!filter) continue
		filters.push(filter)
	}
...
rest.addResponse('get-search-groups', async (view) => {
	...
	const brand = await Bed.getMainBrand(db, md)
	...
	const group_ids = await Bed.getGroupIds(view, nmd, partner)
	...
	// 		if (prop.type == 'value') {
	// 			mdvalues[value_nick] = await Bed.getValueByNick(view, value_nick)
	// 		} else {
	// 			let unit = ''
...
rest.addResponse('get-search-list', async (view) => {	
	...
	view.data.m = md.m
	view.data.md = md
	const {from, where, sort} = await Bed.getmdwhere(view, md, partner)
	...
	const list = await Bed.getModelsByItems(db, base, moditem_ids, partner)
...
rest.addResponse('get-search-sitemap', async (view) => {
	...
	headings[''].items[rel] = {title: conf.root_title}
	...
	const groups = await Bed.getGroups(view)
	items = {}
...
rest.addResponse('set-order', async (view) => {
    ...
    user.model = await Bed.getModelByNick(view, user.brand_nick, user.model_nick, user.partner)
