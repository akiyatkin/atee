...
rest.addVariable('item', async (view) => {
	...
	const item = await Bed.getItemByNick(db, base, brand_nick, model_nick, item_num)
	return item
})
...
rest.addVariable('model', async (view) => {
	...
	const model = await Bed.getModelByNick(view, brand_nick, model_nick, partner)	
	return model
})
// rest.addVariable('partner', async (view) => {
// 	const { user_id } = await view.gets(['user_id'])
// 	const partner = user_id ? await Bed.getPartner(view, user_id) : false
...
const prepareValue = async (db, options, value) => {
	const tree = await Bed.getTree(db, base.visitor)
	const nick = nicked(value)
	let addm = ''
	if (nick) {
		if (nick == 'actions') {
			const vals = options.actions.map(v => nicked(v)).filter(v => v).sort()
			const p = await Bed.getPropByTitle(db, 'Наличие')
			if (p) {
