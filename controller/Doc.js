export class Doc {
	...
	insert (html, div = '', childs = false) {
		// if (div == 'SHOP_PAG') console.log('1231231', html)
	...
	get (div = '', empty = '') {
		...
		if (!div) {
			for (const div in this.divs) {
				if (!this.divs[div][0]) continue //Если в диве пытаемся показать пустой слой, то ошибку пропускаем
